...
type MethodsObj<T> = {
  [key in Methods<T> extends string
    ? Methods<T> // eslint-disable-next-line
  ...
  implements IFlow<TMethods, TSettings>
{
  ...
  /**
   * Registers a method and the function that will run when this method is sent
   * from Flow.
   */
  ...
  /**
   * Runs the function for the current method. Should be called at the end of
